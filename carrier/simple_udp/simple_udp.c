...
static int open_udp_socket(int port)
{
	...
	if (getaddrinfo(NULL, port_str, &hint, &local_addr) != 0) {
		fprintf(stderr, "Failed to translate addrinfo\n");
		abort();
	}

	sd = socket(local_addr[0].ai_family, local_addr[0].ai_socktype, local_addr[0].ai_protocol);
	if (sd < 0) {
		perror("socket()");
		abort();
	}

	if (bind(sd, local_addr[0].ai_addr, local_addr[0].ai_addrlen) < 0) {
...
static void *mod_init(const cJSON * conf, const void *tun)
{
	...
	if (remote_ip != NULL) {
		...
		if (getaddrinfo(remote_ip, port_str, &hint, &local_addr) != 0) {
			fprintf(stderr, "Failed to translate addrinfo\n");
			abort();
		}

		memcpy(ctx->peer_addr, local_addr[0].ai_addr, local_addr[0].ai_addrlen);
		ctx->peer_addr_len = local_addr[0].ai_addrlen;
		freeaddrinfo(local_addr);
	...
	while (ctx->loop) {
		...
		for (i = 0; i < ctx->nr_sockets; ++i) {
			if (pfd[i].revents & POLLIN) {
				...
				memcpy(ctx->peer_addr, &from_addr, from_addr_len);
